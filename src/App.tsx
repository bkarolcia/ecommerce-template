...
const App = () => {
  ...
  const addToBasket = (product: ProductType) => {
    ...
    if (
      basketProducts.find((basketProducts) => basketProducts.id === product.id)
    ) {
      newBasketArray = basketProducts.map((basketProduct) =>
        basketProduct.id === product.id
          ? { ...basketProduct, quantity: basketProduct.quantity! + 1 }
          : basketProduct
      );
    } else {
      newBasketArray = [...basketProducts, { ...product, quantity: 1 }];
    }
  ...
  return (
    <div className="App">
      <Header
        basketProductsQuantity={basketProducts.length}
